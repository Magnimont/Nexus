...
router.get('/:tag', async (req, res) => {
    ...
    if (!authorized) res.status(401).json({err: 'not authorized'});
        ...
        if (exists) {
            ...
            const icon = await db.get(`icon_${exists.user_id}`) || 'https://i.pinimg.com/originals/1f/5a/30/1f5a309ca476474256c3a0049e3499fd.jpg';

            const banner = await db.get(`banner_${exists.user_id}`) || 'https://th.bing.com/th/id/OIP.Ov3O4R-Kv-xvjZcJOtGnnAHaEK?pid=ImgDet&rs=1';
